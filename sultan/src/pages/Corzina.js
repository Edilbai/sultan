...
import korzina from '../img/icons/korzina3.png'
...
const Corzina = ({ corzina }) => {
	...
	return (
		<div>
			...
			{buy.map(obj => (
				<div className='d-flex korzina_card' key={obj.id}>
					<img
						width={225}
						height={235}
						className='img_card'
					...
					<div className='w700'>
						...
						<span>
							...
							at illo veniam ratione sunt, adipisci voluptas hic. Aperiam et
							voluptate provident, iste voluptatum iure temporibus ut accusamus
							voluptas rerum?{' '}
						</span>
					</div>
					<div className='d-flex space-e al-center w70'>
						{' '}
						<button onClick={() => setCount(count - 1)} className='buttons h40'>
							-
						</button>
						<div>{count}</div>
						<button onClick={() => setCount(count + 1)} className='buttons h40'>
					...
					<div className='d-flex space-e al-center w70 '>
						...
						<button onClick={() => remove(obj.id)} className='buttons h40'>
							<img width={15} height={15} src={korzina} alt='' />
						</button>
					</div>
				</div>
			))}
			<hr></hr>
			<button onClick={() => zakaz()} className='buttons'>
				Оформить заказ
			</button>
		</div>
	)
